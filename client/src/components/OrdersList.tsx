...
export const OrdersList = () => {
  ...
  const debouncer = useDebouncer(query, 500);

  useEffect(() => {
    dispatch(fetchOrdersAsync({ page: currentPage, sortBy, customerName: debouncer }));
    const interval = setInterval(() => {
      dispatch(fetchOrdersAsync({ page: currentPage, sortBy, customerName: debouncer }));
    }, 5000);

    return () => clearInterval(interval);
  }, [currentPage, sortBy, debouncer]);
  ...
  return (
    <Paper elevation={3} sx={{ p: 2, height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ mb: 2 }}>
        <SortFilter onSortChange={setSortBy} sortBy={sortBy} query={query} setQuery={setQuery} />
      </Box>
